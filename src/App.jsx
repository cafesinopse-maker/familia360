...
const firebaseConfig = {
  apiKey:            "AIzaSyC_YufrP2s-PXXSNo19A1zDaVINQVXQJas",
  authDomain:        "familia360.firebaseapp.com",
  projectId:         "familia360",
  storageBucket:     "familia360.firebasestorage.app",
  messagingSenderId: "103488954637",
  appId:             "1:103488954637:web:94faef8929a3bec89f7924",
  measurementId:     "G-LDHCNVBP37",
};
...
function Btn({ children, onClick, variant="primary", className="", type="button" }) {
  ...
  return (
    <button type={type} onClick={onClick}
      className={`px-4 py-2 rounded-xl font-semibold text-sm transition ${s[variant]} ${className}`}>
      {children}
    </button>
  );
}
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-600 to-blue-900 flex items-center justify-center p-4">
      <div className="bg-white rounded-3xl w-full max-w-sm p-8 shadow-2xl">
        ...
        <Input label="Senha" type="password" value={pass}
          onChange={e=>setPass(e.target.value)} placeholder="••••••••"/>
        {err && <p className="text-red-500 text-xs mb-3">{err}</p>}
        <Btn onClick={handle} className="w-full mb-3">
  ...
  return (
    <div className="pb-20">
      <div className="bg-gradient-to-br from-blue-600 to-blue-800 text-white px-4 pt-12 pb-6">
        <p className="text-blue-200 text-sm">{days[now.getDay()]}, {now.getDate()} de {mons[now.getMonth()]}</p>
      ...
      <div className="px-4 -mt-4">
          ...
          {pendBills.slice(0,3).map(b=>(
            <Card key={b.id}>
              <div className="flex items-center gap-3">
                <div className="w-3 h-3 rounded-full" style={{background:MEMBERS[b.resp]?.color||"#888"}}/>
        ...
        {activeTks.length>0 && <>
          ...
          {activeTks.slice(0,3).map(t=>(
            <Card key={t.id}>
              <div className="flex items-center gap-3">
                <div className="w-2 h-2 rounded-full" style={{background:PRIO[t.priority]||"#888"}}/>
...
function FinancasScreen({ bills, expenses }) {
  ...
  async function saveBill(data) {
    if(data.id) await upd("bills",data.id,data); else await add("bills",data);
    setModal(null);
  }
  async function saveExpense(data) {
    if(data.id) await upd("expenses",data.id,data); else await add("expenses",data);
    setModal(null);
  }
      ...
      <div className="px-4 mt-4">
        {tab==="fixas" ? <>
          ...
          {bills.map(b=>(
            <Card key={b.id}>
              <div className="flex items-center gap-3">
                <button onClick={()=>upd("bills",b.id,{paid:!b.paid})} className="text-xl">
                  {b.paid?"✅":"⭕"}
                </button>
                ...
                <div key={i} className="flex items-center gap-2 mb-2">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="h-1.5 bg-gray-100 rounded-full mt-1 overflow-hidden">
                      <div className="h-full rounded-full" style={{width:`${Math.round((s.value/gatSegs[0].value)*100)}%`,background:s.color}}/>
                    </div>
                  </div>
...
function AgendaScreen({ events }) {
  ...
  async function saveEvent(data){
    if(data.id) await upd("events",data.id,data); else await add("events",data);
    setModal(null);
  }
    ...
    <div className="pb-20">
      <div className="bg-white px-4 pt-12 pb-3 border-b border-gray-100">
        ...
        <div className="flex items-center gap-3">
          <button onClick={()=>{if(month===0){setMonth(11);setYear(y=>y-1);}else setMonth(m=>m-1)}}
            className="w-8 h-8 rounded-full border border-gray-200 flex items-center justify-center text-gray-500">‹</button>
      ...
      <div className="px-4 mt-4">
        ...
        <div className="flex items-center justify-between mb-2">
          ...
          <button onClick={()=>setModal({data:{date:selStr}})}
            className="text-blue-600 text-sm font-bold">+ Evento</button>
        </div>
...
function TarefasScreen({ tasks }) {
  ...
  async function saveTask(data){
    if(data.id) await upd("tasks",data.id,data); else await add("tasks",data);
    setModal(null);
  }
  function onDrop(colId){ if(drag) upd("tasks",drag,{col:colId}); setDrag(null); }
      ...
      <div className="bg-white px-4 pt-12 pb-3 border-b border-gray-100">
        <div className="flex items-center justify-between">
          ...
          <button onClick={()=>setModal({data:{}})}
            className="bg-blue-600 text-white text-xs font-bold px-3 py-1.5 rounded-xl">+ Nova</button>
        </div>
            ...
            return (
              <div key={col.id} className="rounded-2xl border border-gray-100 min-h-48 flex flex-col"
                ...
                <div className="px-2 py-2 border-b border-white/60">
                  ...
                  <span className="ml-1 text-xs font-bold px-1.5 py-0.5 rounded-full bg-white/70"
                    style={{color:col.color}}>{colTasks.length}</span>
                </div>
                    ...
                    <div key={t.id} draggable
                      ...
                      <div className="flex items-start gap-1.5">
                        <div className="w-2 h-2 rounded-full mt-0.5 flex-shrink-0"
                          style={{background:PRIO[t.priority]||"#888"}}/>
...
// ── FAMÍLIA ───────────────────────────────────────────────────────────────────
function FamiliaScreen({ bills, expenses, events, tasks, user }) {
  const monthPfx=today().slice(0,7);
  return (
    <div className="pb-20">
      <div className="bg-gradient-to-br from-blue-600 to-blue-800 text-white px-4 pt-12 pb-6">
        ...
        {Object.entries(MEMBERS).map(([k,m])=>{
          ...
          return (
            <Card key={k}>
              ...
              <div className="grid grid-cols-3 gap-2">
                {[{label:"Eventos",value:mEvs},{label:"Tarefas",value:mTks},{label:"Gastos",value:"R$"+Math.round(mExp+mBills)}]
                  .map((s,i)=>(
...
function EventModal({ data, onSave, onClose }) {
  ...
  return (
    <Modal title={isEdit?"Editar evento":"Novo evento"} onClose={onClose}>
      ...
      <Select label="Tipo" value={f.type} onChange={e=>setF(p=>({...p,type:e.target.value}))} options={Object.keys(ET)}/>
...
export default function App() {
  ...
  function handleAddModal(type) {
    if(type==="bill"||type==="expense") setTab("finance");
  ...
  return (
    <div className="min-h-screen bg-gray-50 max-w-lg mx-auto relative">
      ...
      {tab==="family"  && <FamiliaScreen bills={bills} expenses={expenses} events={events} tasks={tasks} user={user}/>}
